...
//    }
    ...
    @Transactional
    public Client registerClientWithGoogle(Client client) {
        // Ensure the client is new
    ...
    public void updateLastLogin(Long userId, LocalDateTime loginTime) {
        clientRepository.findById(userId).ifPresent(client -> {
            client.setDerniereConnexion(loginTime);
            clientRepository.save(client);
        });
    }

    @Transactional
    public Client activateClientAccount(String email) {
    ...
    @Transactional
    public Client resetPasswordByEmail(String email, String newPassword) {
        Client client = clientRepository.findByEmail(email)
                .orElseThrow(() -> new RuntimeException("Client not found"));
